...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("scroll", () => {
    if (window.scrollX > 100) {
      // show if user has scrolled horizontally (like Vera's site)
      backArrow.classList.add("show");
    } else if (window.scrollY > 200) {
      // OR show if they've scrolled vertically down a bit
      backArrow.classList.add("show");
    } else {
...
const target = document.querySelector(".projects-header h4");
if (target) projectsObserver.observe(target);
